...
export class SpellBuilder extends BuilderBase {
	...
	__getRaces (cb) {
		...
		const wrpBtnAdd = veT`<div></div>`.vee.appendTo(rowInner);
		veT`<button class="ve-btn ve-btn-xs ve-btn-default">Add Race</button>`
			.vee.appendTo(wrpBtnAdd)
			.vee.onn("click", () => {
	...
	__getRaces__getRaceRow (doUpdateState, raceRows, race) {
		...
		this.constructor.getBtnRemoveRow(doUpdateState, raceRows, out, wrp, "Race").vee.appendTo(wrpBtnRemove);
